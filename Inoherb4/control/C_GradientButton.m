...
@implementation C_GradientButton
...
-(void)addBackImg
{
    img_default=imgbyId(self.tag);
    img_selImg=imgSelectbyId(self.tag);
//    if(self.tag>0)
//    {
//        if(imgPos==TOP)
//            iv_back=[[UIImageView alloc] initWithFrame:CGRectMake(5,5, self.frame.size.width-10, self.frame.size.width-10)];
//        else
//            iv_back=[[UIImageView alloc] initWithFrame:CGRectMake(5,self.frame.size.height-self.frame.size.width+5, self.frame.size.width-10, self.frame.size.width-10)];
//    }
     if(self.tag==-1)
        iv_back=[[UIImageView alloc] initWithFrame:CGRectMake((self.frame.size.width-30)/2,(self.frame.size.height-30)/2, 30, 30) ];
    else
        iv_back=[[UIImageView alloc] initWithFrame:CGRectMake(0,0, self.frame.size.width, self.frame.size.height) ];
    iv_back.image=img_default;
...
-(void)addBackImg:(UIImage*)img
{
    ...
    iv_back=[[UIImageView alloc] initWithFrame:CGRectMake(0,0, self.frame.size.width, self.frame.size.height) ];
    iv_back.image=img_default;
    iv_back.clipsToBounds = YES;
    iv_back.contentMode =  UIViewContentModeScaleAspectFit;
    iv_back.contentScaleFactor=[[UIScreen mainScreen] scale];
    [self addSubview:iv_back];
}
...
-(void)addLeftImg
{
    ...
    iv_back=[[UIImageView alloc] initWithFrame:CGRectMake(15,(self.frame.size.height-40)/2, 40, 40)];
    iv_back.clipsToBounds = YES;
    iv_back.contentMode =  UIViewContentModeScaleAspectFit;
    iv_back.image=img_default;
    [self addSubview:iv_back];
...
-(void)addTopImg
{
    ...
    iv_back=[[UIImageView alloc] initWithFrame:CGRectMake((self.frame.size.width-80)/2,30, 80, 80)];
    iv_back.clipsToBounds = YES;
    iv_back.contentMode =  UIViewContentModeScaleAspectFit;
    iv_back.image=img_default;
    [self addSubview:iv_back];
...
- (void)useImgStyle
{
    ...
    [self.titleLabel setFont:fontBysize(16)];//设置显示字体
//    if(imgPos==TOP)
...
- (void)useLeftImgStyle
{
    ...
    [self setTitleColor:[UIColor lightGrayColor] forState:UIControlStateHighlighted];
    [self.titleLabel setFont:fontBysize(16)];//设置显示字体
    [self setContentEdgeInsets:UIEdgeInsetsMake(0, 60, 0, 0)];//
    [self setContentVerticalAlignment:UIControlContentVerticalAlignmentCenter];
...
//    self.cornerRadius = 6.0f;
    ...
    [self setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [self setTitleColor:[UIColor lightGrayColor] forState:UIControlStateHighlighted];
    [self.titleLabel setFont:fontBysize(15)];//设置显示字体
    [self setContentEdgeInsets:UIEdgeInsetsMake(0, 0, 10, 0)];//
    [self setContentVerticalAlignment:UIControlContentVerticalAlignmentBottom];
...
- (void)useClearStyle
{
    ...
    self.cornerRadius = 9.f;
    [self.titleLabel setFont:fontBysize(12)];//设置显示字体
    [self setTitleColor:[UIColor  whiteColor] forState:UIControlStateNormal];
    [self setTitleColor:[UIColor darkGrayColor] forState:UIControlStateHighlighted];
}
...
- (void)useItemStyle
{
    ...
    NSMutableArray *colors2 = [NSMutableArray arrayWithCapacity:2];
    color = UIColorFromRGB(0x93c34f);
    [colors2 addObject:(id)[color CGColor]];
    color =UIColorFromRGB(0x4e9035);
    [colors2 addObject:(id)[color CGColor]];
...
{
    NSMutableArray *colors = [NSMutableArray arrayWithCapacity:2];
    UIColor *color =col_byIntColor(0x0c519d);
    [colors addObject:(id)[color CGColor]];
    color =col_byIntColor(0x0c519d);
    [colors addObject:(id)[color CGColor]];
    ...
    [self setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [self setTitleColor:[UIColor darkGrayColor] forState:UIControlStateHighlighted];
    [self.titleLabel setFont:fontByBoldsize(18)];//设置显示字体
}

- (void)useAddHocStyle
{
    NSMutableArray *colors = [NSMutableArray arrayWithCapacity:2];
    UIColor *color =col_byIntColor(0x6DCAE9);
    [colors addObject:(id)[color CGColor]];
    color =col_byIntColor(0x6DCAE9);
    [colors addObject:(id)[color CGColor]];
...
//    [self.titleLabel setShadowOffset:CGSizeMake(0, 0.3)];
    [self.titleLabel setFont:fontBysize(14)];
}
...
- (void)useGPSStyle
{
    ...
    UIColor* col=UIColorFromRGB(0xadd655);
...
- (void)useCheckStyle
{
    ...
    self.highlightGradientLocations = [NSMutableArray arrayWithObjects:
                                       ...
                                       [NSNumber numberWithFloat:1.0f],
                                       nil];
    [self setTitleColor:col_byIntColor(0xafcf83) forState:UIControlStateNormal];
    [self setTitleColor:[UIColor darkGrayColor] forState:UIControlStateHighlighted];
    
    self.layer.borderColor=col_byIntColor(0xafcf83).CGColor;
    self.layer.borderWidth=1;
    self.layer.cornerRadius=4;
    //    self.layer.masksToBounds=YES;
    //    [self setTitleShadowColor:[UIColor redColor] forState:UIControlStateNormal];
...
{
    NSMutableArray *colors = [NSMutableArray arrayWithCapacity:2];
    UIColor *color =UIColorFromRGB(0xc6c6c6);
    [colors addObject:(id)[color CGColor]];
    color =UIColorFromRGB(0xc6c6c6);
    [colors addObject:(id)[color CGColor]];

...
@interface PIDrawerViewController () <UIImagePickerControllerDelegate, UIActionSheetDelegate, UINavigationControllerDelegate>
...
- (void)viewDidLoad
{
    ...
    self.backGroundImageView.image = _oriImage;
}
...
- (IBAction)pickEraserButtonPressed:(id)sender
{
    [self.drawerView clear];
}

- (IBAction)saveButtonPressed:(id)sender
{
    UIImage *mergeImage = [self.drawerView mergeImage:_oriImage];
    [_delegate mergeMarkController:self didFinishWithImg:mergeImage];
}
